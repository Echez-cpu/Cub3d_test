...
/*   Updated: 2025/01/01 16:59:19 by pokpalae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	inspect_map_characters(t_game_data *data, char **grid)
{
	int	i;
	int	j;
	...
	while (grid[i] != NULL)
	{
		...
		while (grid[i][j])
		{
			...
			if (!(ft_strchr("10NSEW", grid[i][j])))
				return (is_faulty("an invalid character in the map\n"));
			if (ft_strchr("NSEW", grid[i][j]) && data->first_person.dir != '0')
				return (is_faulty("map has more than one starting position\n"));
			if (ft_strchr("NSEW", grid[i][j]) && data->first_person.dir == '0')
				data->first_person.dir = grid[i][j];
			j++;
		}
		i++;
	}
