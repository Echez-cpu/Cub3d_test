...
/*   Updated: 2025/01/01 17:11:41 by pokpalae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_spawn_position_valid(t_game_data *data, char **map_grid)
{
	...
	if (ft_strlen(map_grid[i - 1]) < (size_t)j || ft_strlen(map_grid[i + 1])
		< (size_t)j || is_white_space(map_grid[i][j - 1]) == 0
		|| is_white_space(map_grid[i][j + 1]) == 0 || is_white_space(map_grid[i
			- 1][j]) == 0 || is_white_space(map_grid[i + 1][j]) == 0)
		return (FAIL);
	return (0);
}
